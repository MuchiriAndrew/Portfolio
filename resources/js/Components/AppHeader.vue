<template>
    <header class="fixed top-0 left-0 right-0 z-40 backdrop-blur transition-transform duration-300 ease-out"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14 md:h-16">
            ...
            <!-- Mobile menu button -->
            <button type="button" class="lg:hidden p-2 text-muted hover:text-white" aria-label="Open menu"
    ...
    <Teleport to="body">
        <Transition name="slide">
            <div v-if="mobileMenuOpen" class="fixed inset-0 z-50 lg:hidden" aria-modal="true" role="dialog">
                ...
                <div class="fixed top-0 right-0 bottom-0 w-full max-w-sm bg-surface shadow-xl flex flex-col">
                    <div class="flex items-center justify-between h-14 px-4 border-b border-gray-700">
                        ...
                        <button type="button" class="p-2 text-muted hover:text-white" aria-label="Close menu"
                    ...
                    <div class="mt-auto p-4 border-t border-gray-700 flex gap-4">
                        <a v-for="link in socialLinks" :key="link.id" :href="link.url || '#'" target="_blank"
                            rel="noopener noreferrer" class="text-muted hover:text-primary"
                            @click="mobileMenuOpen = false">
                            <SocialIcon :platform="link.platform" :logo="link.logo" class="!w-6 !h-6" />
                        </a>
                    </div>
                </div>
            </div>
...
function isActive(item) {
    ...
    if (item.name === 'projects') return page.url.startsWith('/projects')
